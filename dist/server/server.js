...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-ApJrbMvl.js");
...
const manifest = {
  "06bf63aa78f39b038d0e9ab2f3c3fbcdec13644d2f497bc2283101a979cb61e4": {
    functionName: "fetchSession_createServerFn_handler",
    importer: () => import("./assets/session-BmttHp6f.js")
  }
};
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./assets/router-C6je9UwO.js").then((n) => n.r),
...
async function handleServerRoutes({ getRouter, request, url, executeRouter, context, executedRequestMiddlewares }) {
  ...
  if (server2?.handlers && isExactMatch) {
    const handlers2 = typeof server2.handlers === "function" ? server2.handlers({ createHandlers: (d) => d }) : server2.handlers;
    const requestMethod = request.method.toUpperCase();
    const handler = requestMethod === "HEAD" ? handlers2["HEAD"] ?? handlers2["GET"] ?? handlers2["ANY"] : handlers2[requestMethod] ?? handlers2["ANY"];
    isHeadFallback = requestMethod === "HEAD" && handler !== void 0 && !handlers2["HEAD"];
    if (handler) {
...
function TanStackAuth(config) {
  ...
  async function signIn(provider, options = {}) {
    ...
    const url = provider ? `${basePath}/signin/${provider}${paramStr ? `?${paramStr}` : ""}` : `${basePath}/signin${paramStr ? `?${paramStr}` : ""}`;
...
async function refreshAccessToken(token) {
  ...
  try {
    ...
    return {
      ...
      expiresAt: tokenEndpointResponse.expires_in ? Date.now() + tokenEndpointResponse.expires_in * 1e3 : Date.now() + 3600 * 1e3,
...
const { handlers, getSession } = TanStackAuth({
  providers: [
    Zitadel({
  ...
  callbacks: {
    async redirect({ baseUrl }) {
      ...
      return postLoginUrl.startsWith("http") ? postLoginUrl : `${baseUrl}${postLoginUrl}`;
    },
    async jwt({ token, account, user }) {
      if (account && user) {
        return {
          ...
          expiresAt: account.expires_at ? account.expires_at * 1e3 : Date.now() + 3600 * 1e3,
          error: void 0
        };
      }
...
function withCookiesFixed(request, response) {
  const cookies = response.headers.getSetCookie();
  if (cookies.length === 0) return response;
  const nodeRes = request?.runtime?.node?.res;
...
async function handleRequest(request) {
  ...
  if (pathname === "/api/auth/logout/callback") {
    ...
    const cookieNames = cookieHeader.split(";").filter(Boolean).map((c) => c.trim().split("=")[0].trim()).filter((name) => name.startsWith("authjs."));
